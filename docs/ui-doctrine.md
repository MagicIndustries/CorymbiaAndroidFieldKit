...
| Rule | Enforced by |
| --- | --- |
| 4 — nothing blocks capture | Not enforceable by a component or a lint rule, and still a review check in general — but the one place it is most at risk is now pinned by test. The duplicate guard (spec §9.5) is the rule's hardest case, because "a duplicate ought to be prevented" is a reasonable-sounding thought that quietly deletes the rule: accidental double-capture is a real field failure, but so is refusing a legitimate close-spaced pin, and two soil samples a metre apart are a normal thing to record. `apps/fieldkit/app/capture.tsx` therefore runs the guard in the `recorded` phase, *after* `useCapture` has already written the row on the tap, so it can only ever be a warning about what just happened. The test `warns about a close-spaced pin and still records it — it never blocks` (`apps/fieldkit/app/__tests__/capture.test.tsx`) asserts the record's existence **before** the warning's presence, deliberately: a guard moved earlier — onto the tap, or onto the save — then fails on the record's absence, which is the sentence that names what actually went wrong. Verified by breaking the guard into a gate and watching it fail that way. |
| 5 — consistent input affordances | `InputAffordanceRow` (`packages/ui/src/inputs/InputAffordanceRow.tsx`) renders the four affordances in a fixed order taken from the exported `INPUT_AFFORDANCE_ORDER` constant, so the order cannot drift screen to screen; asserted by test. **One exemption is granted, on the screen where this rule matters most, and it is recorded here rather than left to be discovered.** The capture screen (`apps/fieldkit/app/capture.tsx`) does *not* use that component: its `AffordanceTile` reimplements the row's glyphs, its wording, its dashed-versus-solid border, its `affordance-*` and `affordance-*-label` test handles and its `touch.comfortable` sizing. Two facts make that a decision rather than a drift, and both need to be true before it can be undone. **First, the reason it exists:** `InputAffordanceRow` has no *unavailable* state, and this screen has two of them — there is no media table in `@corymbia/data`, so a voice note and a photo have nowhere to be stored, and present-and-disabled is the honest way to show that (rule 3). Teaching the shared component that state was out of scope for the branch that built this screen, which was not permitted to change `packages/ui`. The reimplementation deliberately borrows every visual property rather than inventing a second look, and the order is asserted against the exported constant rather than a literal, so the *rule* holds even where the component does not. **Second, and less obvious: the two rows render different affordances.** `InputAffordanceRow` renders title, description, voice, photo; §9.6's four for a recorded capture are location, title, voice, photo — location is not an input at all here but the fix the capture has already made, shown complete, and description is not among the four. Both rows emit `affordance-title`, `affordance-voice` and `affordance-photo` as testIDs, so putting both on one screen would produce duplicate handles and every `getByTestId` on them would throw. That is the mismatch between §9.6 and this component; it is not resolved here, it needs a decision about which list is canonical, and it is the reason the fix is not simply "use the row and pass a flag". **PLAN 4 (media) is where this attaches:** teach `InputAffordanceRow` the disabled state — or find it no longer needs one — settle the location/description divergence, and replace `AffordanceTile` with the component, rather than growing a third variant of the same four tiles. |
| 7 — help is adjacent, never hover | `HelpAffordance` (`packages/ui/src/help/HelpAffordance.tsx`) is a tappable `?` that opens a modal. There is no `onHover`/tooltip code path in the component at all, so a hover-only help affordance cannot be built with it. **One exemption is granted, and it is recorded here rather than left looking like an oversight:** the capture screen (`apps/fieldkit/app/capture.tsx`) carries its help affordance in the *ready* state only, explaining in plain language what one tap does and why standing still helps. Its *acquiring* state deliberately carries none — rule 17 requires that nothing else is on screen while she stands still over a point, and a `?` beside the two numbers she is standing there for is exactly the "else". Its *recorded* state carries none either, and the earlier wording of this row got the reason wrong: that state does ask something of her — a title, with a text input. What makes it exempt is that the ask explains itself. The field is labelled ("A name for this point", as both the accessibility label and the placeholder), it is optional at every level (rule 3: GPS-only is a complete record), and there is nothing a `?` beside it could say that the box does not already say. Both absences are decisions. A later state on that screen that asks for something whose meaning is *not* on its face — filing to an activity, attaching media — takes a help affordance with it. Asserted by test in both directions (present and opening in ready, absent in acquiring), so neither the affordance nor the exemption can be removed silently. |
...
| 17 — three capture states | Review only, and it needs to stay a review check: nothing a component can enforce distinguishes "the screen hid its panels" from "the screen had no panels". `apps/fieldkit/app/diagnostics.tsx` is the reference implementation — a `phase` of `ready`/`acquiring`/`recorded` chooses between two layouts and three sets of words for one shared capture frame — and its tests assert the transitions directly: that a tap hides the panels, the logs, the choosers and the record list; that completing brings them back; and that taking another reading hides them again. A capture screen added later without those assertions has not met this rule. One thing the reference implementation gets right that is easy to get wrong: the countdown transcript is hidden during *Acquiring* but visible the instant the point is recorded, because it is an artefact the work produces, and a state model that loses a measurement to a tidier layout has failed at the thing it was for. `apps/fieldkit/app/capture.tsx` is the field screen built to this rule, and carries the required assertions: a tap leaves one control and the two numbers (`the acquiring state`), completing brings back the capture number, the final accuracy, the sample count, how the wait ended and the four affordances (`the recorded state`), and taking another reading removes all of them again (`takes another reading back to the ready state, with its panels gone again`). Its *recorded* state names how the wait ended — timer, override or plateau — because spec §9.1.5 was corrected so that a plateau no longer announces itself live, which makes this state the only place that fact ever reaches her. |
