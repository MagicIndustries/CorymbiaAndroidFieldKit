...
| 9 — colour never alone | `ContextStamp` (`packages/ui/src/context-stamp/ContextStamp.tsx`) gives each of the three fix classes — deliberate, ambient, none — a distinct border style (solid vs dashed) *and* distinct wording, never colour alone; its `fix` prop is a discriminated union on `quality`, so an accuracy-less or wrongly-shaped fix cannot even be constructed. `InputAffordanceRow` applies the same two-channel pattern to completion state: a solid border plus a check mark appended to the label, not a colour change alone. `CaptureDial` (`packages/ui/src/capture/CaptureDial.tsx`) is what this row named the traffic-light frame for before the frame was deleted (spec §9.2): its grade word — `GOOD FIX` / `FAIR FIX` / `POOR FIX` — is always rendered beside the colour, at every phase, so the colour is never read alone. **One channel did not migrate with it**, and that is recorded here rather than left to be found by accident: the deleted frame additionally dashed its border on a poor fix, and `CaptureDial` has no border of its own to dash — a poor fix at the dial today reads by colour and word only, the same two-channel minimum the rule requires, but one channel short of what `ContextStamp` and `InputAffordanceRow` both do. Spec §9.2.2 still calls for "a dashed treatment when poor"; closing that gap is unfinished work, not a decision made against the rule, and belongs with whichever task next touches `CaptureDial`. |
